  ...
  return(
    <form onSubmit={(event) => handleSubmit(event)} onBlur={() => props.updateGearItem(props.gearItem)}>
      <div className={"row " + (props.index === 0 ? "" : "top-dotted")}>
        ...
        <div className="col-1"><FontAwesome name='times-circle' className="delete" size="lg" onClick={() => { if (window.confirm('Are you sure you want to delete item?')) props.deleteGearItem(props.gearItem) }}/></div>
      </div>
